...
def generate_patched_dll(filename):
    ...
    orig, patched = bytes.fromhex("EDFD425CF978"), bytes.fromhex("EDFD42CBF978")
    ...
    print(f"Patched {filename} (original overwritten).")
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: patch.py <key_lib> <lib_to_patch_1> [<lib_to_patch_2> ...]")
        sys.exit(1)
    ...
    with open(key_lib, "w") as f:
        f.write(serialized)

    print(f"Saved new license to {key_lib}!")

    for lib in sys.argv[2:]:
        generate_patched_dll(lib)
